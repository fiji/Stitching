	...
	public ImagePlus open( final boolean virtual )
	{
		...
		try
		{
			...
			else
			{
				...
				if ( impIj != null )
				{
					IJ.log( "Used ImageJ1 tiff opener." );
					imp = new ImagePlus[]{ impIj };
				}
				else
			...
			if ( imp.length > 1 )
			{
				...
				for ( final ImagePlus i : imp )
					i.close();

				return null;
			}
			if ( imp[ 0 ].getNSlices() == 1 )
				if ( imp[ 0 ].getNFrames() > 1 )
				{
					imp[ 0 ].setDimensions( imp[ 0 ].getNChannels(), imp[ 0 ].getNFrames(), 1 );
